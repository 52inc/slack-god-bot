...
class GodBot extends SlackBot {
  ...
  _reply(message) {
    ...
    console.log(isLeaderboardMessage);
    ...
    if (!isMentioningBot && isRankMessage) {
      console.log('User changing the rank of another user...');
    ...
    if (isMentioningBot && !isRankMessage) {
      console.log('Responding directly to a user...');
  ...
  _post(channel, params = { as_user: true }) {
    return (message) => {
  ...
  _postLeaderboard(channel) {
    ...
    self.postMessageToChannel(channel.name, leaderBoardMessage, {
      as_user: true
    });
  }

  _postEvents(channel, message, user) {
    let self = this;
  }

  _postFoodSpecials(channel, message, user) {
    let self = this;
  }

  _postGame(channel, message, user) {
    let self = this;
  }
  ...
  _postRepost(channel, message, user) {
    let self = this;

  }

  _postRestaurants(channel, message, user) {
    let self = this;
  ...
  _isLeaderboardMessage(message) {
    let allowedSpaces = 2;
    let botId = this.user.id;
    let botIdRegExp = new RegExp(`${botId}`, 'i');
    let leaderboardRegExp = new RegExp('leaderboard', 'i');
    let maxMessageLength = botId.length + ('leaderboard').length + allowedSpaces;

    return (
      leaderboardRegExp.test(message.text) &&
      botIdRegExp.test(message.text) &&
      message.text >= maxMessageLength
    );
  }
